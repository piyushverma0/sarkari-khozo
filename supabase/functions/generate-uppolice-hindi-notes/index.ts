...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🚀 Generating UP Police Hindi RAW content for:', note_id, 'Topic:', topic || 'General')
...
Generate comprehensive study material for UP Police General Hindi exam.

**Cover these key topics:**
1. Hindi Grammar (Vyakaran)
2. Hindi Literature (Sahitya)
3. Hindi Comprehension (Avabodhan) (Ancient, Medieval, Modern, Freedom Struggle)
...
7. Uttar Pradesh State Hindi (History, Geography, Culture, Administration, UP Police)
...
Return detailed markdown-formatted study material (NOT JSON). This raw content will be processed into structured notes.`
    ...
    const topicContext = topic ? `Focus specifically on: ${topic}` : 'Provide comprehensive General Hindi coverage for UP Police exam'

    const userPrompt = `Generate comprehensive UP Police Constable 2026 General Hindi study material.

${topicContext}

Include:
- All major Hindi topics relevant to UP Police exam
- Current affairs from last 6 months
- UP State Hindi (10-15% weightage)
...
Create detailed, exam-focused markdown content.`
    ...
    console.log('🤖 Calling AI for UP Police Hindi RAW content generation...')
    ...
    console.log('📝 Triggering summarization for UP Police Hindi notes...')
