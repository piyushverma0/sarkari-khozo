...
serve(async (req) => {
  ...
  try {
    ...
    if (!note.summary && !note.key_points && !note.raw_content && !note.structured_content) {
      throw new Error("Note has no content. Please ensure the note has been fully processed.");
    }

    // Build content for AI prompt
    // Priority: structured_content > raw_content
    let detailedContent = "N/A";

    if (note.structured_content?.sections) {
      // If structured content with sections exists, format it nicely
      try {
        let formattedContent = "";
        note.structured_content.sections.forEach((section: any) => {
          formattedContent += `\n${section.title}:\n${section.content}\n`;
        });
        detailedContent = formattedContent;
      } catch (e) {
        console.warn("Failed to parse structured_content sections, trying stringify");
        detailedContent = JSON.stringify(note.structured_content, null, 2);
      }
    } else if (note.structured_content) {
      // If structured_content exists but no sections, stringify it
      try {
        detailedContent = JSON.stringify(note.structured_content, null, 2);
      } catch (e) {
      ...
      detailedContent = note.raw_content.substring(0, 10000);
    }
