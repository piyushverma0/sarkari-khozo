...
serve(async (req) => {
  ...
  try {
    ...
    if (!note_id || !user_id || !quiz_type) {
      return new Response(
        JSON.stringify({ error: 'note_id, user_id, and quiz_type are required' }),
        {
          status: 400,
  ...
  "questions": [
    {
      ...
      "options": ["A", "B", "C", "D"] (for MCQ/True-False only),
...
Generate exactly ${question_count} questions. Return ONLY the JSON.`
    ...
    try {
      ...
      throw new Error(`Failed to parse quiz: ${parseError instanceof Error ? parseError.message : String(parseError)}`)
    }
