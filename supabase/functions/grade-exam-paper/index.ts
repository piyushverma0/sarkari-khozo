...
interface QuestionFeedback {
  ...
  max_marks: number
  feedback: string
  correct_answer_reference?: string | null
}
...
serve(async (req) => {
  ...
  try {
    ...
    const sectionScores: SectionScore[] = formattedPaper.sections.map((section: any) => {
      const sectionQuestionNumbers = section.questions.map((q: any) => q.question_number)
      const sectionFeedbacks = questionFeedbacks.filter(fb =>
        sectionQuestionNumbers.includes(fb.question_number)
      )
      ...
      return {
        ...
        percentage: sectionTotalMarks > 0 ? (sectionMarksObtained / sectionTotalMarks) * 100 : 0
      }
    })

    // Calculate overall percentage and grade
    const percentage = examPaper.total_marks > 0 
      ? (totalMarksObtained / examPaper.total_marks) * 100 
      : 0
...
Be fair and constructive. Award partial marks for partially correct answers.`
  ...
  let aiResponse: { content: string; tokensUsed?: any }
  ...
  const marksAwarded = Math.min(
    Math.max(0, gradingData.marks_awarded || 0),
    question.marks
  )
