...
export async function callAI(options: AICallOptions): Promise<AIResponse> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Both AI models failed. Sonar: ${sonarError instanceof Error ? sonarError.message : String(sonarError)}, GPT-4: ${gptError instanceof Error ? gptError.message : String(gptError)}`)
    }
  }
}
...
export async function callAIWithFile(options: AIFileUploadOptions): Promise<AIResponse> {
  ...
  try {
    ...
    console.warn('⚠️ Sonar Pro file processing failed:', sonarError instanceof Error ? sonarError.message : String(sonarError))
    ...
    try {
      ...
      console.error('❌ GPT-4-turbo file processing also failed:', gptError instanceof Error ? gptError.message : String(gptError))
      throw new Error(`Both AI models failed for file processing. Sonar: ${sonarError instanceof Error ? sonarError.message : String(sonarError)}, GPT-4: ${gptError instanceof Error ? gptError.message : String(gptError)}`)
    }
  }
}
...
async function callGPT4Turbo(options: AICallOptions): Promise<AIResponse> {
  const { systemPrompt, userPrompt, temperature, maxTokens, responseFormat } = options
...
async function callGPT4TurboWithFile(options: AIFileUploadOptions): Promise<AIResponse> {
  const { systemPrompt, userPrompt, fileBuffer, mimeType, displayName, temperature, maxTokens, responseFormat } = options
