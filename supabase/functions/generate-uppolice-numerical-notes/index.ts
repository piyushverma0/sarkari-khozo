...
serve(async (req) => {
  ...
  try {
    ...
    // Build comprehensive UP Police Numerical system prompt
    const systemPrompt = `You are an expert UP Police Constable 2026 exam preparation specialist with deep knowledge of the Numerical & Mental Ability syllabus for UP Police recruitment.

**EXAM CONTEXT:**
- **Exam:** UP Police Constable 2026
- **Total Vacancies:** 32,679+ positions
- **Subject:** Numerical & Mental Ability (सामान्य ज्ञान)
- **Question Pattern:** Multiple Choice Questions (MCQs)
- **Focus Areas:** Current Affairs, History, Geography, Polity, Economy, Science, UP State Numerical, National & International Affairs
...
7. **Uttar Pradesh State Numerical (उत्तर प्रदेश सामान्य ज्ञान)**
...
{
  "title": "UP Police Numerical: [Topic Name] - Exam Preparation Notes",
...
Remember: Create comprehensive, exam-focused notes that help students ace the UP Police Constable 2026 Numerical & Mental Ability section.`;
...
Requirements:
- Cover all relevant Numerical syllabus topics for UP Police exam
- Include current affairs from last 6 months
- Add UP State Numerical (10-15% weightage)
...
Create detailed, exam-focused content that maximizes student success in UP Police Numerical section.`;
    ...
    try {
      ...
      throw new Error(`Failed to parse generated content: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
    }
    ...
    const { error: updateError } = await supabase
      .from("study_notes")
      .update({
        title: structuredContent.title || "UP Police Numerical Notes",
