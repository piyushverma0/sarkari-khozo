...
const EXAM_BOARD_INFO: Record<string, { name: string; session: string }> = {
  'CBSE': {
...
serve(async (req) => {
  ...
  let requestBody: FormatPaperRequest | null = null

  try {
    requestBody = await req.json()
    const { exam_paper_id } = requestBody as FormatPaperRequest
    ...
    // Get board info
    const boardInfo = EXAM_BOARD_INFO[examPaper.exam_type] || {
    ...
    const validation = validateExamPaper(
      partialPaper.sections,
    ...
    return new Response(
      JSON.stringify({
        ...
        paper: {
          ...
          sections: completePaper.sections.map((s: ExamSection) => ({
    ...
    try {
      const authHeader = req.headers.get('Authorization')
      if (authHeader && requestBody?.exam_paper_id) {
        const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
        ...
        await supabase
          ...
          .eq('id', requestBody.exam_paper_id)
      }
    } catch (e) {
...
function formatGeneralInstructions(examType: string, customInstructions: string[]): string[] {
  ...
  const examSpecificInstructions: Record<string, string[]> = {
    'CBSE': [
  ...
  const allInstructions = [
    ...
    ...customInstructions.filter(inst =>
      !baseInstructions.includes(inst) && !specificInstructions.includes(inst)
    )
  ]
...
function validateExamPaper(
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    const sectionMarks = section.questions.reduce((sum: number, q) => sum + (q.marks || 0), 0)
  ...
  if (marksVariance > allowedVariance) {
    errors.push(
      `Total marks mismatch: got ${totalMarks}, expected ${expectedTotalMarks} (variance: ${marksVariance})`
    )
  }
